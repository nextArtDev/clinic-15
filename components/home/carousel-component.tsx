...
const Carousel: React.FC<CarouselProps> = ({
  ...
  const [slidesrArr, setSlidesArr] = useState<ReactNode[]>([])
  const plugins = []
...
export const Slider: React.FC<SliderProps> = ({
  ...
  const hasAdded = useRef(false) // Add this ref

  useEffect(() => {
    if (thumbnail && !hasAdded.current) {
      hasAdded.current = true
      setSlidesArr((prev: ReactNode[]) => [...prev, thumbnail])
    }
  }, [setSlidesArr, thumbnail]) // Keep dependencies but guard with ref
    ...
    <div className="overflow-hidden my-0.5" ref={emblaThumbsRef}>
      <div className="flex flex-row gap-0.5">
        {slidesrArr.map((slide, index) => (
          <div
            key={`thumb-${index}`}
            // className={`min-w-0 w-full xl:h-24 aspect-auto border-2 rounded-md cursor-pointer ${
            className={`min-w-0 w-full xl:h-24 aspect-auto border-none rounded-md cursor-pointer ${
              index === selectedIndex
...
// Rest of the code (useDotButton, usePrevNextButtons, useSelectedSnapDisplay) remains unchanged
